...
interface User {
    name: string; // Change to fit your API response
}
...
const CodeFetcher: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch('http://127.0.0.1:5000/getName'); // Ensure this endpoint is correct
                if (!response.ok) {
                ...
                const snippet = result.map(user => user.name).join('\n');
