...
export default function InformativeRisk() {
  ...
  const fetchApplicationsByRisk = async () => {
    try {
      ...
      const informativeRiskData = data.data
        .filter((item: RiskRatingGroup) => item._id.toLowerCase() === 'informative')
        .map((item) => ({
          ...item,
      ...
      setFilteredApplications(informativeRiskData);
    } catch (error) {
            ...
            <Card elevation={4} sx={{ p: 2, borderRadius: '12px', backgroundColor: '#424242' }}>
              <CardContent>
                ...
                <Typography variant="h4" sx={{ fontWeight: 'bold', color: '#90caf9' }}>
                  {riskRatings.reduce((acc, group) => acc + (group["Total Applications Scanned"] || 0), 0) || 'N/A'}
                </Typography>
              </CardContent>
            </Card>
          </Grid>
            ...
            <Card elevation={4} sx={{ p: 2, borderRadius: '12px', backgroundColor: '#424242' }}>
              <CardContent>
                ...
                <Typography variant="h4" sx={{ fontWeight: 'bold', color: '#e57373' }}>
                  {riskRatings.reduce((acc, group) => acc + (group["Total Findings"] || 0), 0) || 'N/A'}
                </Typography>
              </CardContent>
            </Card>
          </Grid>
