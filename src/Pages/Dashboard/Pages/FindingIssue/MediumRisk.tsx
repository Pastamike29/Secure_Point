...
export default function MediumRisk() {
  ...
  const fetchApplicationsByRisk = async () => {
    try {
      ...
      const mediumRiskData = data.data
        .filter((item: RiskRatingGroup) => item._id.toLowerCase() === 'medium')
        .map((item) => ({
          ...item,
      ...
      setFilteredApplications(mediumRiskData);
    } catch (error) {
  ...
  return (
    <DashboardLayout title="Medium Risk Applications">
