...
interface ApplicationDetails {
  ...
  "Finding Issues Count": number; // Count of issues for the application
}
...
interface RiskRatingGroup {
  ...
  "Total Findings": number; // Total issues across all applications
}
...
export default function CriticalRisk() {
  ...
  const fetchApplicationsByRisk = async () => {
    try {
      ...
      const criticalRiskData = data.data.filter((item: RiskRatingGroup) => item._id === 'Critical');
                ...
                <Table>
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell><strong>Total Finding Issues</strong></TableCell> {/* Matches backend data */}
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {group.applications
                      .sort((a, b) => b["Finding Issues Count"] - a["Finding Issues Count"]) // Descending order
                      .map((app, index) => (
                        <TableRow key={index}>
